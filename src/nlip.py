...
def nlip_encode_text(message:str, control=False,language='english' ):
        return NLIP_Message(control=control, format=AllowedFormats.text, subformat=language, content = message)

def nlip_encode_dict(contents:dict, control=False):
    return NLIP_Message(control=control, format=AllowedFormats.structured, subformat='json', content=contents)


def nlip_encode_exception(err: Exception):
     return NLIP_Message(control=True, format='text', subformat='English', content = f'Sorry, I can not process your request!! {err}')
