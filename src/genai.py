...
from common_utils import errors as err
...
class GenAI:
    ...
    def generate_templated(self, prompt_template:str, prompt_args:dict, **kwargs) -> str:
...
class OllamaClient(GenAI):
    ...
    def _base_httpx_call(self, apicall:Literal["generate", "embeddings", "chat"], priority_data:dict, **kwargs):
...
class SimpleGenAI:
    ...
    def generate_templated(self, model:str, prompt_template:str, prompt_args:dict, **kwargs) -> str:
       prompt = prompt_template.format(**prompt_args)
...
'''
class StatefulGenAI: 
    def __init__(self, host:str='localhost', port:int=11434, model:str='mistral'):
