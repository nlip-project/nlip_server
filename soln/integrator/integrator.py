...
from src.genai import SimpleGenAI
from src.server import setup_server
from soln.integrator import config as cfg
...
class IntegratorSession(nlip.NLIP_Session):
    ...
    def __init__(self, servers:list):
        ...
        self.models_dict = { x[cfg.SERVER_NAME]: x[cfg.SERVER_MODEL] for x in self.server_cfg}
        
    def create_server(self, server_cfg:dict):
        return SimpleGenAI(server_cfg[cfg.SERVER_HOST], server_cfg[cfg.SERVER_PORT])

    def start(self):
        self.server_dict = {x[cfg.SERVER_NAME]:self.create_server(x) for x in self.server_cfg}
    ...
    def voted_response(self, question:str, responses:dict):
        votes = [(x, responses[x], self.count_votes(x, question, responses[x])) for x in responses.keys()]
