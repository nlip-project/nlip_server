...
class UnImplementedError(PrivateException):
    def __init__(self, function_name:str, class_name:str=None, ):
        if class_name is None:
            message = f'function {function_name} is not implemented'
        else:
            message = f'function {function_name} in class {class_name} is not implemented'
        super().__init__(message)
...
class DuplicateEntryError(Exception):
    def __init__(self, className:str, parameter:str):
        super().__init__(f'Tried to create a new instance of {className} but an instance with name {parameter} already exists')

class MissingEntry(Exception):
    def __init__(self, objectType:str, name:str):
        super().__init__(f'There is no {objectType} with name of {name} in persistent store')
...
class MissingKeyError(Exception):
    def __init__(self, field_name:str, this_dict:dict):
        super().__init__(f'Error: Expected key {field_name} in dictionary but it is not present in dictionary {this_dict}')

class MissingParameterError(Exception):
    def __init__(self, field_name:str, function_name:str, this_dict:dict):
        super().__init__(f'Error: Expected parameter {field_name} to be spcified when calling function {function_name} but it is not present in {this_dict}')

class MissingCallableFunction(Exception):
    def __init__(self, function_name:str, class_name:str):
        super().__init__(f'Error: function {function_name} is not defined on objects of class {class_name}')